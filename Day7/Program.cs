...
var rootDirectory = new ElfDirectory { Name = "/" };

var currentDirectory = rootDirectory;
currentDirectory = BuildDirectories(input, rootDirectory, currentDirectory);

int maxSizeThreshold = 100000;
int sum = GetDirectorySizesAboveThreshold(maxSizeThreshold, rootDirectory);
...
int GetSizeOfDirectoryToRemove()
{
    ...
    GetAllDirectorySizes(rootDirectory, directorySizes);

    int requiredSpace = 30000000;
    int totalSpace = 70000000;

    int spaceToFree = Math.Abs(requiredSpace - (totalSpace - rootDirectory.DirectorySize));
    var ordered = directorySizes.OrderBy(x => x);

    return ordered.First(x => x >= spaceToFree);
}

int GetDirectorySizesAboveThreshold(int maxSizeThreshold, ElfDirectory root)
{
    int sum = 0;
    if (root.DirectorySize <= maxSizeThreshold)
        sum += root.DirectorySize;

    foreach (var dir in root.Directories)
    {
        sum += GetDirectorySizesAboveThreshold(maxSizeThreshold, dir);
    }
...
ElfDirectory BuildDirectories(string input, ElfDirectory root, ElfDirectory currentDirectory)
{
    foreach (var line in input.Split(Environment.NewLine))
    {
        ...
        else if (line.StartsWith(InputConstants.ChangeDirectory))
        {
            ...
            currentDirectory = currentDirectory.Directories.First(x => x.Name == dirNme);
        }
